...
public class SupportTalk implements ConfigurationEventHandler {
	...
	public void execute(ConfigurationEvent event) {
		...
		if (event.getSubOperation().equals("set")) {
			event.getMessage().editMessageEmbeds(LanguageEngine.getMessageEmbed(guild, user, this, "defchannel")).queue();
			AwaitTask.forMessageReceival(guild, user, event.getChannel(),
					...
					e -> {
						...
						event.getMessage().editMessageEmbeds(LanguageEngine.getMessageEmbed(guild, user, this, "setsuccess").replaceDescription("{channel}", channel.getAsMention())).queue();
						return;
					}, null).append();
		}
		if (event.getSubOperation().equals("clear")) {
			ConfigLoader.INSTANCE.getGuildConfig(guild).put("supporttalk", 0L);
			event.getMessage().editMessageEmbeds(LanguageEngine.getMessageEmbed(guild, user, this, "clearsuccess")).queue();
		}
	}
