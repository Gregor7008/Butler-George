...
public class Penalties implements ConfigurationEventHandler {
	...
	public void execute(ConfigurationEvent event) {
		...
		if (event.getSubOperation().equals("list")) {
			...
			if (response != null) {
				event.getMessage().editMessageEmbeds(LanguageEngine.getMessageEmbed(guild, user, this, "list").replaceDescription("{list}", response)).queue();
			}
		}
	...
	private void deletepenalties(ConfigurationEvent event) {
		String response = this.listpenalties(event);
		if (response != null) {
			event.getMessage().editMessageEmbeds(LanguageEngine.getMessageEmbed(guild, user, this, "remlist").replaceDescription("{list}", response)).queue();
			...
			AwaitTask.forMessageReceival(guild, user, event.getChannel(),
					e -> {try {
					    	  ...
					    	  event.getMessage().editMessageEmbeds(LanguageEngine.getMessageEmbed(guild, user, this, "nodelval")).queue();
					      }}).append();
		}
	}
	...
	private void addpenalties1(ConfigurationEvent event) {
	    StringSelectMenu menu = StringSelectMenu.create("selpen")
				...
				.build();
		event.getMessage().editMessageEmbeds(LanguageEngine.getMessageEmbed(guild, user, this, "add1")).setComponents(ActionRow.of(menu)).queue();
	...
	private void addpenalties2(String plannedpunish, StringSelectInteractionEvent event, ConfigurationEvent op) {
			event.editMessageEmbeds(LanguageEngine.getMessageEmbed(guild, user, this, "add2")).setComponents().queue();
	...
	private void addpenalties3(String plannedpunish, String warnings, ConfigurationEvent op) {
		...
		try {
			...
			event.getMessage().editMessageEmbeds(LanguageEngine.getMessageEmbed(guild, user, this, "error")).queue();
			return;
		} catch (JSONException e) {}
		switch (plannedpunish) {
		case "rr":
			event.getMessage().editMessageEmbeds(LanguageEngine.getMessageEmbed(guild, user, this, "add3role")).queue();
			AwaitTask.forMessageReceival(guild, user, op.getChannel(),
					...
					e -> {penalties.put(warnings, new JSONArray().put(plannedpunish).put(e.getMessage().getMentions().getRoles().get(0).getId()));
						  event.getMessage().editMessageEmbeds(LanguageEngine.getMessageEmbed(guild, user, this, "successrole")).queue();}, null).append();
			break;
		case "tm":
			event.getMessage().editMessageEmbeds(LanguageEngine.getMessageEmbed(guild, user, this, "add3time")).queue();
			AwaitTask.forMessageReceival(guild, user, op.getChannel(),
					e -> {try {
							  Integer.valueOf(e.getMessage().getContentRaw());
							  return true;
					...
					e -> {penalties.put(warnings, new JSONArray().put(plannedpunish).put(e.getMessage().getContentRaw()));
						  event.getMessage().editMessageEmbeds(LanguageEngine.getMessageEmbed(guild, user, this, "successtempmute")).queue();}, null).append();
			break;
		case "pm":
			penalties.put(warnings, new JSONArray().put(plannedpunish).put("0"));
			event.getMessage().editMessageEmbeds(LanguageEngine.getMessageEmbed(guild, user, this, "successmute")).queue();
			break;
		case "ki":
			penalties.put(warnings, new JSONArray().put(plannedpunish).put("0"));
			event.getMessage().editMessageEmbeds(LanguageEngine.getMessageEmbed(guild, user, this, "successkick")).queue();
			break;
		case "tb":
			event.getMessage().editMessageEmbeds(LanguageEngine.getMessageEmbed(guild, user, this, "add3time")).queue();
			AwaitTask.forMessageReceival(guild, user, op.getChannel(),
					...
					e -> {penalties.put(warnings, new JSONArray().put(plannedpunish).put(e.getMessage().getContentRaw()));
						  event.getMessage().editMessageEmbeds(LanguageEngine.getMessageEmbed(guild, user, this, "successtempban")).queue();}, null).append();
			break;
		case "pb":
			penalties.put(warnings, new JSONArray().put(plannedpunish).put("0"));
			event.getMessage().editMessageEmbeds(LanguageEngine.getMessageEmbed(guild, user, this, "successban")).queue();
			break;
		default:
			event.getMessage().editMessageEmbeds(LanguageEngine.getMessageEmbed(guild, user, this, "fatal")).queue();
		}
	}
	...
	private String listpenalties(ConfigurationEvent event) {
		...
		if (current.isEmpty()) {
			event.getMessage().editMessageEmbeds(LanguageEngine.getMessageEmbed(guild, user, this, "nopenalties")).queue();
			return null;
		}
