...
public class LevelRewards implements ConfigurationEventHandler {
	...
	public void execute(ConfigurationEvent event) {
		...
		if (event.getSubOperation().equals("add")) {
			event.getMessage().editMessageEmbeds(LanguageEngine.getMessageEmbed(guild, user, this, "deflevel")).queue();
			AwaitTask.forMessageReceival(guild, user, event.getChannel(),
					...
					e -> {int neededLevel = Integer.parseInt(e.getMessage().getContentRaw());
						  event.getMessage().editMessageEmbeds(LanguageEngine.getMessageEmbed(guild, user, this, "defrole")).queue();
						  AwaitTask.forMessageReceival(guild, user, event.getChannel(),
								  ...
								  r -> {long roleID = r.getMessage().getMentions().getRoles().get(0).getIdLong();
								  	    ...
								  	    event.getMessage().editMessageEmbeds(LanguageEngine.getMessageEmbed(guild, user, this, "addsuccess")
			...
			AwaitTask.forMessageReceival(guild, user, event.getChannel(),
					...
					e -> {int level = Integer.parseInt(e.getMessage().getContentRaw());
						  try {
							  ...
							  event.getMessage().editMessageEmbeds(LanguageEngine.getMessageEmbed(guild, user, this, "noreward")).queue();
							  return;
						  }
	...
	private void listrewards(ConfigurationEvent event, JSONObject levelrewards) {
		...
		if (levelrewards.isEmpty()) {
			event.getMessage().editMessageEmbeds(LanguageEngine.getMessageEmbed(guild, user, this, "norewards")).queue();
			return;
		}
		...
		event.getMessage().editMessageEmbeds(LanguageEngine.getMessageEmbed(guild, user, this, "list").replaceDescription("{list}", sB.toString())).queue();
	}
}