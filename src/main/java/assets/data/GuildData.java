...
public class GuildData implements DataContainer {
	...
	public GuildData(JSONObject data) throws EntityNotFoundException {
		...
		Guild guild_object = Bot.getAPI().getGuildById(guild_id);
		if (guild_object == null) {
		    throw new EntityNotFoundException(ReferenceType.GUILD, guild_id);
		} else {
            ...
            if (channel != null) {
                ...
                reaction_role_sub_data.keySet().forEach(messageId -> {
                    ...
                    if (message != null && !reaction_role_data.isEmpty()) {
                        reactionRoleSubMap.put(Long.valueOf(messageId), new ReactionRoleData(channel, message, reaction_role_data));
                    }
                });
...
//  Other
	...
	public void setReactionRolesByChannel(TextChannel channel, ConcurrentHashMap<Message, ReactionRoleData> reaction_roles) {
