...
public class Goodbye implements ActionRequest {
	...
	public void execute(Action event) {
		...
		if (event.getSubAction().getName().equals("set")) {
			...
			event.replyEmbeds(LanguageEngine.fetchMessage(guild, user, this, "/commands/moderation/goodbye:setsuccess")).queue();
			return;
		}
		if (event.getSubAction().getName().equals("off")) {
			ConfigLoader.getGuildConfig(guild).put("goodbyemsg", "");
			event.replyEmbeds(LanguageEngine.fetchMessage(guild, user, this, "/commands/moderation/goodbye:offsuccess")).queue();
			return;
		}
		if (event.getSubAction().getName().equals("test")) {
			...
			if (goodbyemsgraw != "") {
				...
				event.replyEmbeds(LanguageEngine.fetchMessage(guild, user, this, "/commands/moderation/goodbye:testsuccess")).queue();
			} else {
				event.replyEmbeds(LanguageEngine.fetchMessage(guild, user, this, "/commands/moderation/goodbye:nonedefined")).queue();
			}
		}
	}
