...
public class CustomChannelRoles implements ActionRequest {
	...
	public void execute(Action event) {
		...
		if (event.getSubAction().getName().equals("set")) {
			ccroles.clear();
			ccroles.put(roleID);
			event.replyEmbeds(LanguageEngine.fetchMessage(guild, user, "/commands/moderation/customchannelroles:setsuccess"));
			return;
		}
		if (event.getSubAction().getName().equals("add")) {
			ccroles.put(roleID);
			event.replyEmbeds(LanguageEngine.fetchMessage(guild, user, "/commands/moderation/customchannelroles:addsuccess"));
			return;
		}
		if (event.getSubAction().getName().equals("remove")) {
			ConfigLoader.removeValueFromArray(ccroles, roleID);
			event.replyEmbeds(LanguageEngine.fetchMessage(guild, user, "/commands/moderation/customchannelroles:remsuccess"));
			return;
		}
		if (event.getSubAction().getName().equals("clear")) {
			ccroles.clear();
			event.replyEmbeds(LanguageEngine.fetchMessage(guild, user, "/commands/moderation/customchannelroles:clearsuccess"));
			return;
		}
	}
