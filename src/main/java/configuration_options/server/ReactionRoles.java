...
public class ReactionRoles implements ConfigurationEventHandler {
					  ...
					  AwaitTask.forMessageReceival(guild, user, event.getChannel(),
							  ...
							  m -> {this.msgid = e.getMessage().getContentRaw();
							  	    if (event.getSubOperation().equals("add")) {
							  	    	event.getMessage().editMessageEmbeds(LanguageEngine.fetchMessage(guild, user, this, "defineAddRoles")).complete();
							  	    	this.defineAddRoles();
							  	    	return;
							  	    }
	...
	private void defineAddEmojis(List<Role> roles) {
		...
		event.getMessage().editMessageEmbeds(LanguageEngine.fetchMessage(guild, user, this, "defineAddEmojis").replaceDescription("{role}", role.getAsMention())).queue();
		AwaitTask.forReactionAdding(guild, user, message,
	...
	private void addReactions() {
		event.getMessage().editMessageEmbeds(LanguageEngine.fetchMessage(guild, user, this, "adding")).complete();
		...
		Bot.INSTANCE.getTimer().schedule(new TimerTask() {
			@Override
			public void run() {
				event.getMessage().editMessageEmbeds(LanguageEngine.fetchMessage(guild, user, this, "success")).queue();
			}
		}, 1500);
	}
}