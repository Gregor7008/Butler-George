...
public class Move implements Command{
	...
	public void perform(SlashCommandInteractionEvent event) {
		...
		if (ConfigLoader.getGuildConfig(guild).getLong("supporttalk") == 0) {
			event.replyEmbeds(AnswerEngine.fetchMessage(guild, user, "/commands/moderation/move:nochannel").convert()).queue();
			return;
		}
		if (!guild.getMember(event.getOption("member").getAsUser()).getVoiceState().inAudioChannel()) {
			event.replyEmbeds(AnswerEngine.fetchMessage(guild, user, "/commands/moderation/move:memnotconn").convert()).queue();
			return;
		}
		...
		if (st.getMembers().contains(event.getMember())) {
			...
			event.replyEmbeds(AnswerEngine.fetchMessage(guild, user, "/commands/moderation/move:success").convert()).queue();
		} else {
			event.replyEmbeds(AnswerEngine.fetchMessage(guild, user, "/commands/moderation/move:notconnected").convert()).queue();
		}
	}
	...
	public String getHelp(Guild guild, User user) {
		return AnswerEngine.getRaw(guild, user, "/commands/moderation/move:help");
	}
}