...
public class Welcome implements Command{
	...
	public void perform(SlashCommandInteractionEvent event) {
		...
		if (event.getSubcommandName().equals("set")) {
			...
			event.replyEmbeds(AnswerEngine.fetchMessage(guild, user,"/commands/moderation/welcome:setsuccess").convert()).queue();
			return;
		}
		if (event.getSubcommandName().equals("off")) {
			ConfigLoader.getGuildConfig(guild).put("welcomemsg", "");
			event.replyEmbeds(AnswerEngine.fetchMessage(guild, user,"/commands/moderation/welcome:offsuccess").convert()).queue();
			return;
		}
		if (event.getSubcommandName().equals("test")) {
			...
			if (welcomemsgraw != "") {
				...
				event.replyEmbeds(AnswerEngine.fetchMessage(guild, user, "/commands/moderation/welcome:testsuccess").convert()).queue();
			} else {
				event.replyEmbeds(AnswerEngine.fetchMessage(guild, user,"/commands/moderation/welcome:nonedefined").convert()).queue();
			}
		}
	}
	...
	public String getHelp(Guild guild, User user) {
		return AnswerEngine.getRaw(guild, user, "/commands/moderation/welcome:help");
	}
}