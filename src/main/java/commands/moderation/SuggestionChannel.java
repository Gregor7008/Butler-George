...
public class SuggestionChannel implements Command{
	...
	public void perform(SlashCommandInteractionEvent event) {
		...
		if (event.getSubcommandName().equals("set")) {
			...
			event.replyEmbeds(AnswerEngine.fetchMessage(guild, user, "/commands/moderation/suggestionchannel:setsuccess").convert()).queue();
			return;
		}
		if (event.getSubcommandName().equals("clear")) {
			ConfigLoader.getGuildConfig(guild).put("suggestionchannel", 0L);
			event.replyEmbeds(AnswerEngine.fetchMessage(guild, user, "/commands/moderation/suggestionchannel:clearsuccess").convert()).queue();
		}
	}
	...
	public String getHelp(Guild guild, User user) {
		return AnswerEngine.getRaw(guild, user, "/commands/moderation/suggestionchannel:help");
	}
}