...
public class SupportChannel implements Command{
	...
	public void perform(SlashCommandInteractionEvent event) {
		...
		if (event.getSubcommandName().equals("set")) {
			...
			event.replyEmbeds(AnswerEngine.run.fetchMessage(guild, user, "/commands/moderation/supportchannel:setsuccess").convert()).queue();
			return;
		}
		if (event.getSubcommandName().equals("clear")) {
			ConfigLoader.run.getGuildConfig(guild).put("suggestionchannel", Long.valueOf(0));
			event.replyEmbeds(AnswerEngine.run.fetchMessage(guild, user, "/commands/moderation/supportchannel:clearsuccess").convert()).queue();
		}
	}
	...
	public String getHelp(Guild guild, User user) {
		return AnswerEngine.run.getRaw(guild, user, "/commands/moderation/supportchannel:help");
	}
}