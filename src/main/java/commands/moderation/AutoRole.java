...
public class AutoRole implements Command {
	...
	public void perform(SlashCommandInteractionEvent event) {
		...
		if (event.getSubcommandName().equals("add")) {
			...
			event.replyEmbeds(AnswerEngine.fetchMessage(guild, user,"/commands/moderation/autorole:addsuccess").convert()).queue();;
			return;
		}
		if (event.getSubcommandName().equals("remove")) {
			...
			event.replyEmbeds(AnswerEngine.fetchMessage(guild, user,"/commands/moderation/autorole:removesuccess").convert()).queue();
			return;
		}
	...
	public String getHelp(Guild guild, User user) {
		return AnswerEngine.getRaw(guild, user, "/commands/moderation/autorole:help");
	}
	...
	private void listroles(SlashCommandInteractionEvent event) {
		...
		if (autoroles.isEmpty()) {
			event.replyEmbeds(AnswerEngine.fetchMessage(guild, user,"/commands/moderation/autorole:noautoroles").convert()).queue();;
			return;
		}
		...
		event.replyEmbeds(AnswerEngine.fetchMessage(guild, user, "/commands/moderation/autorole:list").replaceDescription("{list}", sB.toString()).convert()).queue();
	}
}