...
public class DefaultAccessRoles implements Command{
	...
	public void perform(SlashCommandEvent event) {
		...
		if (!event.getMember().hasPermission(Permission.MANAGE_SERVER)) {
			event.replyEmbeds(AnswerEngine.getInstance().fetchMessage(guild, user, "/commands/moderation/defaultaccessroles:nopermission")).queue();
			return;
		}
		event.replyEmbeds(AnswerEngine.getInstance().fetchMessage(guild, user, "/commands/moderation/defaultaccessroles:sendroles")).queue();
		Bot.INSTANCE.getWaiter().waitForEvent(GuildMessageReceivedEvent.class,
				...
				e -> {List<Role> roles = e.getMessage().getMentionedRoles();
					  ...
					  event.getTextChannel().sendMessageEmbeds(AnswerEngine.getInstance().fetchMessage(guild, user, "/commands/moderation/defaultaccessroles:success")).queue();},
				2, TimeUnit.MINUTES,
	...
	public String getHelp(Guild guild, User user) {
		return AnswerEngine.getInstance().getRaw(guild, user, "/commands/moderation/defaultaccessroles:help");
	}
