...
public class Warning implements Command{
	...
	public void perform(SlashCommandInteractionEvent event) {
		...
		if (event.getSubcommandName().equals("add")) {
			...
			ConfigLoader.run.getUserConfig(guild, iuser).getJSONArray("warnings").put(reason);
			event.replyEmbeds(AnswerEngine.run.fetchMessage(guild, user,"/commands/moderation/warning:success").convert()).queue();
			try {
				final String repl = reason;
				iuser.openPrivateChannel().queue(channel -> {
					channel.sendMessageEmbeds(AnswerEngine.run.fetchMessage(guild, iuser, "/commands/moderation/warning:pm")
		...
		if (event.getSubcommandName().equals("remove")) {
			if (this.listwarnings(event)) {
				event.getChannel().sendMessageEmbeds(AnswerEngine.run.fetchMessage(guild, user, "/commands/moderation/warning:remsel").convert()).queue();
				...
				waiter.waitForEvent(MessageReceivedEvent.class,
						...
						e -> {JSONArray warnings = ConfigLoader.run.getUserConfig(guild, iuser).getJSONArray("warnings");
							  int w = Integer.parseInt(e.getMessage().getContentRaw());
							  ConfigLoader.run.removeValueFromArray(warnings, warnings.get(w-1));
							  channel.sendMessageEmbeds(AnswerEngine.run.fetchMessage(guild, user, "/commands/moderation/warning:remsuccess")
						...
						1, TimeUnit.MINUTES,
						() -> {channel.sendMessageEmbeds(AnswerEngine.run.fetchMessage(guild, user,"/commands/moderation/warning:timeout").convert()).queue(response -> response.delete().queueAfter(3, TimeUnit.SECONDS));});
			}
		}
	...
	public String getHelp(Guild guild, User user) {
		return AnswerEngine.run.getRaw(guild, user, "/commands/moderation/warning:help");
	}
	...
	private boolean listwarnings(SlashCommandInteractionEvent event) {
		...
		User user = event.getUser();
		final User iuser = event.getOption("user").getAsUser();
		JSONArray warnings = ConfigLoader.run.getUserConfig(guild, iuser).getJSONArray("warnings");
		if (warnings.isEmpty()) {
			event.replyEmbeds(AnswerEngine.run.fetchMessage(guild, user,"/commands/moderation/warning:nowarnings").convert()).queue();
			return false;
		}
		...
		event.replyEmbeds(AnswerEngine.run.fetchMessage(guild, user, "/commands/moderation/warning:list")
