...
public class Warning implements Command{
	...
	public void perform(SlashCommandInteractionEvent event) {
		...
		if (event.getSubcommandName().equals("add")) {
			...
			event.replyEmbeds(LanguageEngine.fetchMessage(guild, user,"/commands/moderation/warning:success").convert()).queue();
			try {
				final String repl = reason;
				iuser.openPrivateChannel().queue(channel -> {
					channel.sendMessageEmbeds(LanguageEngine.fetchMessage(guild, iuser, "/commands/moderation/warning:pm")
		...
		if (event.getSubcommandName().equals("remove")) {
			if (this.listwarnings(event)) {
				event.getChannel().sendMessageEmbeds(LanguageEngine.fetchMessage(guild, user, "/commands/moderation/warning:remsel").convert()).queue();
				...
				waiter.waitForEvent(MessageReceivedEvent.class,
						...
						e -> {JSONArray warnings = ConfigLoader.getMemberConfig(guild, iuser).getJSONArray("warnings");
							  ...
							  channel.sendMessageEmbeds(LanguageEngine.fetchMessage(guild, user, "/commands/moderation/warning:remsuccess")
							  ...
							  warnings.remove(w-1);},
						1, TimeUnit.MINUTES,
						() -> {channel.sendMessageEmbeds(LanguageEngine.fetchMessage(guild, user,"/commands/moderation/warning:timeout").convert()).queue(response -> response.delete().queueAfter(3, TimeUnit.SECONDS));});
			}
		}
	...
	private boolean listwarnings(SlashCommandInteractionEvent event) {
		...
		if (warnings.isEmpty()) {
			event.replyEmbeds(LanguageEngine.fetchMessage(guild, user,"/commands/moderation/warning:nowarnings").convert()).queue();
			return false;
		}
		...
		event.replyEmbeds(LanguageEngine.fetchMessage(guild, user, "/commands/moderation/warning:list")
