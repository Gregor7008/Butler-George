...
public class Move implements Command {
	...
	public void perform(SlashCommandInteractionEvent event) {
		...
		if (ConfigLoader.getGuildConfig(guild).getLong("supporttalk") == 0) {
			event.replyEmbeds(LanguageEngine.fetchMessage(guild, user, this, "/commands/moderation/move:nochannel").convert()).queue();
			return;
		}
		if (!guild.getMember(event.getOption("member").getAsUser()).getVoiceState().inAudioChannel()) {
			event.replyEmbeds(LanguageEngine.fetchMessage(guild, user, this, "/commands/moderation/move:memnotconn").convert()).queue();
			return;
		}
		...
		if (st.getMembers().contains(event.getMember())) {
			...
			event.replyEmbeds(LanguageEngine.fetchMessage(guild, user, this, "/commands/moderation/move:success").convert()).queue();
		} else {
			event.replyEmbeds(LanguageEngine.fetchMessage(guild, user, this, "/commands/moderation/move:notconnected").convert()).queue();
		}
	}
