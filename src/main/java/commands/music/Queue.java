...
public class Queue implements Command {
	...
	public void perform(SlashCommandInteractionEvent event) {
		...
		if (!self.getVoiceState().inAudioChannel()) {
			event.replyEmbeds(LanguageEngine.fetchMessage(guild, user,"/commands/music/queue:notconnected").convert()).queue();
			return;
		}
		if (member.getVoiceState().getChannel() != self.getVoiceState().getChannel()) {
			event.replyEmbeds(LanguageEngine.fetchMessage(guild, user,"/commands/music/queue:nopermission").convert()).queue();
			return;
		}
		if (queue.isEmpty()) {
			event.replyEmbeds(LanguageEngine.fetchMessage(guild, user,"/commands/music/queue:noqueue").convert()).queue();
			return;
		}
		...
		final String temp1[] = LanguageEngine.getRaw(guild, user, "/commands/music/queue:list").split(";");
